...
namespace BetterRomance
{
    ...
    internal class ComplexOrientationDisplay : UIColumn
    {
        ...
        internal ComplexOrientationDisplay(Gender gender, bool romance)
        {
            this.gender = gender;
        ...
        void MakeGenderBox()
        {
            ...
            GenderAttractionChances chances = BetterRomanceMod.settings.GenderToChances(gender, romance);
            ...
            if (gender != Gender.Male)
            {
                buttonRow.Add(NewElement.Button(() => chances.CopyFrom(BetterRomanceMod.settings.GenderToChances(gender - 1, romance)))
                    .WithLabel("WBR.MatchAboveButton".Translate), name: prefix + "MatchAboveButton");
            }
            if (gender != (Settings.NonBinaryActive ? (Gender)3 : Gender.Female))
            {
                buttonRow.Add(NewElement.Button(() => chances.CopyFrom(BetterRomanceMod.settings.GenderToChances(gender + 1, romance)))
        ...
        void MakeOrientationBox()
        {
            ...
            OrientationChances chances = BetterRomanceMod.settings.GenderToOrientation(gender, romance);
            BetterRomanceMod.settings.SetUpOrientationChanceSection(section, romance, chances);
            orientationBox.AddSpace(8f);
            UIRow buttonRow = orientationBox.AddRow(gap: 0f, name: prefix + "OrientationButtonRow");
            if (gender != Gender.Male)
            {
                buttonRow.Add(NewElement.Button(() => chances.CopyFrom(BetterRomanceMod.settings.GenderToOrientation(gender - 1, romance)))
